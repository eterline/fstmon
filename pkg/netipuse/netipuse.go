...
func AddrNext(ip netip.Addr) netip.Addr {
	addr := vec128From16(ip.As16()).addOne()
	if ip.Is4() {
		if uint32(addr.loBit) == 0 {
			// Overflowed.
			return netip.Addr{}
		}
		return addr.IPv4()
		...
		return addr.IPv6().WithZone(ip.Zone())
	}
}
...
func AddrPrior(ip netip.Addr) netip.Addr {
	addr := vec128From16(ip.As16())
	if ip.Is4() {
		if uint32(addr.loBit) == 0 {
			return netip.Addr{}
		}
		return addr.subOne().IPv4()
	} else {
		if addr.isZero() {
			return netip.Addr{}
		}
		return addr.subOne().IPv6().WithZone(ip.Zone())
	}
}
...
type prefixMaker func(a vec128, bits uint8) netip.Prefix
...
func (r PoolRange) AppendPrefixes(dst []netip.Prefix) []netip.Prefix {
	...
	a := vec128From16(r.from.As16())
	b := vec128From16(r.to.As16())
	return appendRangePrefixes(dst, r.prefixFrom128AndBits, a, b)
}

func (r PoolRange) prefixFrom128AndBits(a vec128, bits uint8) netip.Prefix {
	var ip netip.Addr
	if r.from.Is4() {
		bits -= 12 * 8
		ip = a.IPv4()
...
func comparePrefixes(a, b vec128) (common uint8, aZeroBSet bool) {
	...
	return common, (a.xor(a.and(m)).isZero() &&
		b.or(m) == vec128{^uint64(0), ^uint64(0)})
}
...
func (r PoolRange) Prefix() (p netip.Prefix, ok bool) {
	...
	from128 := vec128From16(r.from.As16())
	to128 := vec128From16(r.to.As16())
...
func appendRangePrefixes(dst []netip.Prefix, makePrefix prefixMaker, a, b vec128) []netip.Prefix {
...
func ParseSubnet(netStr string) (netip.Prefix, error) {
	pfx, err := netip.ParsePrefix(netStr)
	if err == nil {
		return pfx, nil
	}

	addr, err := netip.ParseAddr(netStr)
	if err != nil {
